...
function MobileBottomNav() {
  const { t } = useLanguage();
  return (
    <nav className="fixed inset-x-3 bottom-3 z-50 grid grid-cols-5 rounded-2xl border border-border bg-white/90 p-1 shadow-[0_16px_35px_-18px_rgba(194,122,134,0.3)] backdrop-blur md:hidden" aria-label="Mobile navigation">
      ...
      <Link to="/booking" className="-mt-5 flex min-w-0 flex-col items-center justify-center gap-1 rounded-2xl bg-primary px-1 py-3 text-[10px] font-bold text-primary-foreground shadow-[0_10px_20px_-10px_#c27a86]"><CalendarCheck className="h-5 w-5" /><span className="truncate">{t.nav.book}</span></Link>
      <a href="https://wa.me/972598291081" target="_blank" rel="noopener noreferrer" className="flex min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[10px] font-semibold text-card-foreground"><MessageCircle className="h-4 w-4 text-primary" /><span className="truncate">{t.nav.contact}</span></a>
